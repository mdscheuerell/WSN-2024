...
$$\LARGE \text{Var}(popn) = \frac{\text{Var}(env)}{DD} ~~ \Rightarrow ~~ stability =  \frac{1}{\text{Var}(popn)} = \frac{DD}{\text{Var}(env)}$$
...
# How does one estimate these matrices?
...
# Sensitivity of $\text{det}(\mathbf{B})^2$

## .blue.center[ to a change in the effect of species _j_ on species _i_ ]

$$\LARGE \frac{\partial \text{det}(\mathbf{B})^2}{\partial \mathbf{B}_{ij}} = \left[ 2 ~ \text{det}(\mathbf{B}) (\mathbf{B}^{-1})^\top \right]_{ij}$$
...
### .blue[ Food web studies often focus on pairwise interactions ]

### .blue[ Examining the stability of the community is important as well ]
