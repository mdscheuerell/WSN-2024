...
# Estimating species interactions<br>has long history in ecology
...
par(mfrow = c(1,3), mai = c(0.5, 0.5, 0.5, 0.5), omi=c(0, 0, 0, 0)) 
...
par(mai = c(0.7, 0, 0.5, 0), omi = rep(0, 4))
...
text("-0.5", x = xm[2] * 0.95, y = y0 - (y0 - y1) / 2, pos = 2,
     cex = 2, col = "#80cdc1")
text("0.6", x = xm[2] * 1.35, y = ymt,
     cex = 2, col = "#80cdc1")
## arrows
arrows(x0 = xm[2] * 0.95, y0 = y0, y1 = y1,
       col = "#80cdc1", lwd = 3, length = 0.12)
## circle
symbols(x = xm[2] * 1.35, y = ymt, circles = c(2),
...
text("0.7", x = xm[2] * 1.05, y = y0 - (y0 - y1) / 2, pos = 4,
     cex = 2, col = "#488fdf")
text("0.8", x = xm[2] * 1.35, y = ymb,
     cex = 2, col = "#488fdf")
## arrows
arrows(x0 = xm[2] * 1.05, y0 = y1, y1 = y0,
       col = "#488fdf", lwd = 3, length = 0.12)
## circle
symbols(x = xm[2] * 1.35, y = ymb, circles = c(2),
...
> ## .blue[ The size of a population changes over time ]
...
> ## .blue[ A community is composed of multiple interacting populations ]
...
# Model for community dynamics<sup>*</sup>
...
### .citation[ <sup>*</sup>in log space with matrix notation] 
...
# Estimating community stability
...
MM <- MASS::mvrnorm(TT, mu = c(0, 0), Sigma = matrix(c(1, 0.7, 0.7, 1), 2, 2))

par(mai = c(1, 1, 0, 0), omi = c(0, 0, 0, 0))
plot(MM[,1], MM[,2], lwd = 2, type = "l",
        yaxt = "n", xaxt = "n",
        ylab = "", xlab = "")
mtext("Population x",
...
## .center[which implies a .blue[population] is .blue[more stable]]

## .center[ when the strength of .orange[density-dependence increases]]
...
## .blue.center[ Our multivariate model of community dynamics ]

$$\LARGE \mathbf{x}_t = \mathbf{r} +  \mathbf{B} \mathbf{x}_{t-1} + \mathbf{w}_t \\
~ \\
\LARGE \mathbf{w}_t \sim \text{MVN}(\mathbf{0}, \mathbf{Q})$$

## .blue.center[ where the stationary variance is ]

$$\LARGE \mathbf{\Sigma_{\infty}} = \mathbf{B} \mathbf{\Sigma_{\infty}} \mathbf{B}^\top + \mathbf{Q}$$

### .gray.citation[Ives et al (2003) _Ecol Monogr_]
